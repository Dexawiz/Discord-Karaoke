...
function setupWidget() {
    ...
    // Ждем, пока SC появится в window
    ...
    // ВАЖНО: Команды можно слать ТОЛЬКО внутри READY
    widget.bind(SC.Widget.Events.READY, () => {
        console.log('✅ SoundCloud Widget Bridge Established');
        isWidgetReady = true;
        
        // Теперь можно безопасно запрашивать длительность
        widget.getDuration((ms: number) => {
            const durationDisplay = document.getElementById('duration-display');
            if (durationDisplay) durationDisplay.textContent = formatTime(ms / 1000);
        });
    });

    // Обработка ошибок самого виджета
    widget.bind(SC.Widget.Events.ERROR, (err: any) => {
...
function showJoinButton(serverTime: number) {
    ...
    document.getElementById('real-join-btn')?.addEventListener('click', () => {
		...
		widget.seekTo(diffInSeconds * 1000);
		widget.play();
...
function handleState(state: any) {
	...
	if(!isWidgetReady || !widget) {
		console.warn('Widget not ready yet, skipping state update');
		return;
	}
    ...
    if (state.isPlaying) {
        ...
        widget.getPosition((ms: number) => {
            ...
            const drift = Math.abs(current - diffInSeconds);

            if (drift > 1.2) {
                widget.seekTo(diffInSeconds * 1000); // МИЛЛИСЕКУНДЫ
            }
			widget.play();
        });
...
const initMusic = () => {
	...
	progressContainer?.addEventListener('click', (e) => {
        widget.getDuration((durationMs: number) => {
            ...
            const targetTime = clickPercent * (durationMs / 1000); // Переводим в секунды для сервера
	...
	pauseBtn?.addEventListener('click', () => {
        widget.isPaused((paused: boolean) => {
            ws.send(JSON.stringify({ type: paused ? 'PLAY' : 'PAUSE' }));
        });
    });
};
...
if (volumeSlider) {
    volumeSlider.addEventListener('input', (event) => {
        ...
        if (widget) {
            widget.setVolume(value * 100); // В SoundCloud громкость от 0 до 100
        }
        if (volumeDisplay) {
...
setInterval(() => {
    ...
    widget.isPaused((paused: boolean) => {
        if (!paused) {
            widget.getPosition((ms: number) => {
                widget.getDuration((totalMs: number) => {
                    handleTimeUpdate(ms / 1000, totalMs / 1000);
                });
            });
        }
    });
